...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        // задаем название сцены
        guard let windowScene = (scene as? UIWindowScene) else { return }
        // из windowScene создаем window
        let myWindow = UIWindow(windowScene: windowScene )
        // теперь у window назначаем корневым контороллером тапБарКонтроллер
    ...
    func createFirstNavigationController() -> UINavigationController {
        let someNavigationController = UINavigationController(rootViewController: NoteViewController())
        // у UINavigationController есть свойство .tabBarItem которое будет передано в UITabBarController
        someNavigationController.tabBarItem = UITabBarItem(title: "Заметки", image: UIImage(systemName: "text.alignleft"), tag: 0)
    ...
    func createSecondNavigationController() -> UINavigationController {
        let someNavigationController = UINavigationController(rootViewController: InfoViewController())
        // у UINavigationController есть свойство .tabBarItem которое будет передано в UITabBarController
        someNavigationController.tabBarItem = UITabBarItem(title: "Info", image: UIImage(systemName: "person.crop.circle"), tag: 1)
        return someNavigationController
    }
